...
const files = [
  'src/app/guides/best-helicopter-iceland/products.ts',
  'src/app/guides/best-helicopter-iceland/page.tsx',
  'src/app/guides/best-helicopter-iceland/TopThreeCards.tsx',
  'src/app/guides/best-helicopter-iceland/HelicopterWidget.tsx',
  'src/data/guides.ts',
...
const pageBundle = [
  text['src/app/guides/best-helicopter-iceland/products.ts'],
  text['src/app/guides/best-helicopter-iceland/page.tsx'],
  text['src/app/guides/best-helicopter-iceland/TopThreeCards.tsx'],
].join('\n');
const guideEntry = text['src/data/guides.ts'].split("slug: 'best-helicopter-iceland'")[1]?.split("slug: '")[0] || '';
...
for (const id of ['t418541', 't46584', 't418522']) {
...
const cards = text['src/app/guides/best-helicopter-iceland/TopThreeCards.tsx'];
if (!cards.includes('TrackedGYGLink')) errors.push('TopThreeCards must wrap CTAs in TrackedGYGLink');
if (!cards.includes('helicopter-which-tour-cta')) errors.push('TopThreeCards missing which-tour CTA section');

const widget = text['src/app/guides/best-helicopter-iceland/HelicopterWidget.tsx'];
...
const page = text['src/app/guides/best-helicopter-iceland/page.tsx'];
...
if (guideEntry.includes('£') || guideEntry.includes('&pound;')) {
  errors.push('guides.ts helicopter entry has leftover £');
}

const products = text['src/app/guides/best-helicopter-iceland/products.ts'];
...
for (const url of ['d0957f109b48ba08', '59f7362412099', 'd4ac718215c24729']) {
  if (!products.includes(url)) errors.push(`missing this tour's own GYG photo hash ${url}`);
}

const llms = text['src/app/llms.txt/route.ts'];
if (!llms.includes('/guides/best-helicopter-iceland')) {
  errors.push('llms.txt route missing /guides/best-helicopter-iceland');
}
