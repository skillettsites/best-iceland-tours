...
export const alt = 'Which whale watching tour is actually the best in 2026?';
export const size = { width: 1200, height: 630 };
export const contentType = 'image/png';

export default async function OGImage() {
  const guide = getGuideBySlug('best-whale-watching-iceland');
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ fontSize: 44, fontWeight: 800, color: 'white', lineHeight: 1.2, maxWidth: 900, display: 'flex' }}>
            {guide?.title || 'Which whale watching tour is actually the best in 2026?'}
          </div>
        </div>
        <div style={{ fontSize: 20, color: '#dbeafe', display: 'flex' }}>
          best-iceland-tours.com
        </div>
      </div>
    ),
    {
